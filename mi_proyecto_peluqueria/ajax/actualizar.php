<?php
    ...
    // Solo permitir acceso a administradores o superadministradores
    if (!in_array($_SESSION['rol'], ['admin', 'superadmin'])) {
        header("Location: ../admin/dashboard.php?error=No autorizado");
        exit();
    }

    // Determinar la acción (qué entidad se va a actualizar)
    ...
    switch ($action) {
        ...
        case 'producto':
            ...
            // Obtener imagen actual y procesar nueva si se sube
        ...
        // ---------- ACTUALIZAR USUARIO (cliente o administrador) ----------
        case 'usuario':
        case 'admin':
            ...
            // Evitar email duplicado (excepto el propio usuario)
            ...
            if (!empty($password)) {
                if (strlen($password) < 8 || !preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/', $password)) {
                    header("Location: ../admin/dashboard.php?error=La contraseña debe tener al menos 8 caracteres, una mayúscula, una minúscula y un número");
                    exit();
                }
            ...
            // Redirigir según el origen (gestionar_admins.php o dashboard.php)
            $redirect = "../admin/dashboard.php";
            if ($action == 'admin') {
